...
- Shortcode/block attributes: `view` (tree|grid|list), `root` (member id), `department`, `location`. Department/location are documented to users as name-or-slug; `resolve_term()` also accepts a numeric term id, which is what the block stores and what the toolbar round-trips to the render endpoint — keep that branch. Filters prune the tree: matching nodes keep their mutual reporting lines; matching descendants of non-matching members are promoted.